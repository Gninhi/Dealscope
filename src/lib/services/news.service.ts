// ─── News Service ─────────────────────────────────────────────────
// Pure business logic for fetching, categorizing, and caching M&A news.
...
export function getSourceTier(host: string): string {
...
/**
 * Sanitize text decoded from HTML entities: strip any script tags,
 * event handlers, or other dangerous HTML patterns.
 */
  ...
  for (const feedUrl of feeds) {
    try {
      ...
      while ((match = itemRegex.exec(text)) !== null) {
        ...
        // Sanitize decoded HTML entities (prevent XSS from decoded content)
        const safeTitle = sanitizeDecodedHtml(rawTitle).slice(0, MAX_TITLE_LENGTH);
        const safeDesc = sanitizeDecodedHtml(cleanDesc).slice(0, MAX_SNIPPET_LENGTH);
        ...
        if (safeTitle && link) {
          try {
            ...
            items.push({
              ...
              date: pubDate,
              category: categorize(safeTitle, safeDesc, ''),
    ...
    const results = await Promise.all(
      batch.map(async ({ q, cat }) => {
        ...
        return items.map((r: any) => ({
          title: sanitizeDecodedHtml(r.title || '').slice(0, MAX_TITLE_LENGTH),
          snippet: sanitizeDecodedHtml(r.snippet || '').slice(0, MAX_SNIPPET_LENGTH),
          url: r.url || '',
          hostName: r.hostName || '',
  ...
  if (query) {
    ...
    if (!refresh && cache.has(ck)) {
      ...
      if (Date.now() - c.ts < TTL)
        return { results: c.data, total: c.data.length, category: 'veille' };
    }
    ...
    return { results: items, total: items.length, category: 'veille' };
  }
